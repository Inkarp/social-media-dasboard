 ...
 *
 * which means the broken page reaches production looking fine. Lint does not
 * catch it either — it is a framework constraint, not a lint rule. So it gets
 * its own check.
 */
...
for (const file of walk(ROOT)) {
  ...
  // `export type` / `export interface` don't exist in plain JS, but a JSDoc
  // `@typedef` never becomes a real export either — this pattern is kept for
  // safety in case a `.jsx` file is ever hand-edited with TS syntax by mistake.
