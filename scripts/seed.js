...
import { currentFy, fyLabel, fyOptions } from '../src/lib/fy.js'
...
if (!url || !serviceKey) {
  console.error(
    [
      'Missing credentials.',
      '',
      'scripts/seed.js needs these in .env.local:',
...
/**
 * @typedef {Object} SeedFile
 * @property {{ name: string, email: string | null }[]} product_managers
 * @property {{
 *   name: string, group_name: string, product_manager: string | null,
 *   country: string | null, brand_color: string, is_active: boolean
 * }[]} principals
 */
 ...
 * @param {string} step
 * @param {{ message: string, details?: string | null } | null} error
 */
function assertOk(step, error) {
  if (!error) return
 ...
 */
async function seedProductManagers(seed) {
...
/**
 * @param {SeedFile} seed
 * @param {Map<string, string>} managers
 */
async function seedPrincipals(seed, managers) {
