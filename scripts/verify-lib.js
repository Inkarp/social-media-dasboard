 ...
 *   npm run verify:lib
 *
 * src/lib/fy.js is the single most load-bearing file in the project: every
 ...
 * scripts/verify-schema.js — here the concern is the arithmetic itself.
 */
...
import { channelListLabel } from '../src/lib/channels.js'
import { compareGroups, GROUPS } from '../src/lib/groups.js'
import { safeInternalPath } from '../src/lib/safe-path.js'
import { isPostStatus, POST_STATUSES } from '../src/lib/status.js'
...
/** @type {[string, number, import('../src/lib/fy.js').Quarter][]} */
const boundaryCases = [
  ['2025-01-01', 2024, 4],
  ['2025-03-01', 2024, 4],
...
/** @type {[import('../src/lib/fy.js').Quarter, string, string][]} */
...
/** @type {import('../src/lib/fy.js').DateRange[]} */
const tiled = /** @type {import('../src/lib/fy.js').Quarter[]} */ ([1, 2, 3, 4]).map((q) => quarterRange(2025, q))
check('Q1 starts where the year starts', toDateOnly(/** @type {import('../src/lib/fy.js').DateRange} */ (tiled[0]).start), toDateOnly(fyRange(2025).start))
check('Q4 ends where the year ends', toDateOnly(/** @type {import('../src/lib/fy.js').DateRange} */ (tiled[3]).end), toDateOnly(fyRange(2025).end))
...
for (let i = 0; i < 3; i++) {
  const current = /** @type {import('../src/lib/fy.js').DateRange} */ (tiled[i])
  const next = /** @type {import('../src/lib/fy.js').DateRange} */ (tiled[i + 1])
  const endDay = toDateOnly(current.end)
  const nextStartDay = toDateOnly(next.start)
  const dayAfterEnd = toDateOnly(new Date(current.end.getTime() + 86_400_000))
  check(
...
check('FY 2025 has 12 months, April first, March last', {
  ...
  first: toDateOnly(/** @type {Date} */ (fyMonths(2025)[0])),
  last: toDateOnly(/** @type {Date} */ (fyMonths(2025)[11])),
