 ...
 * in SQL rather than in JavaScript — the generated fy/quarter columns and the
...
import { deriveQuarterTargets, fyOf, quarterOf, parseDateOnly } from '../src/lib/fy.js'
 ...
 */
function checkThrows(description, ran, message) {
  checks++
  if (!ran) {
    console.log(`  ok    ${description}`)
  } else {
...
async function main() {
  ...
  console.log('\nGenerated columns (fy / quarter) — and agreement with src/lib/fy.js')
  ...
  /** @type {{ description: string, sql: string }[]} */
  const attempts = [
    {
  ...
  // scripts/seed.js upserts with `onConflict: 'name'` on both tables and
  ...
  for (const table of /** @type {const} */ (['principals', 'product_managers'])) {
    let ran = false
    let message
    try {
      if (table === 'principals') {
    ...
    check(`${table} upsert on (name) works — seed.js conflict target is valid`, upserted, true)
  }
  ...
  /* ---------------------------------------------------------------------- */
  /* Quarter target derivation must match fy.js exactly                     */
  /* ---------------------------------------------------------------------- */
  ...
  for (const yearly of [0, 1, 2, 3, 4, 7, 10, 50, 51, 99, 100]) {
    ...
    check(
      `yearly ${yearly} splits the same in SQL and JavaScript`,
  ...
  /** @param {string} args */
  const rollup = async (args) =>
    /** @type {{ rows: RollupRow[] }} */ (await db.query(`select * from dashboard_rollup(${args})`)).rows
  ...
  check(
    ...
    (({ planned, implemented, pending }) => ({ planned, implemented, pending }))(
      byPrincipal.find((r) => r.label === 'Radleys') ?? /** @type {RollupRow} */ ({}),
    ),
  ...
  check(
    ...
    (({ planned, implemented, pending }) => ({ planned, implemented, pending }))(
      byPrincipal.find((r) => r.label === 'Maccor') ?? /** @type {RollupRow} */ ({}),
    ),
  ...
  check(
    ...
    (({ planned, implemented, pending }) => ({ planned, implemented, pending }))(
      managers.find((r) => r.label === 'Praveen Reddy') ?? /** @type {RollupRow} */ ({}),
    ),
  ...
  check(
    ...
    (({ planned, implemented, pending }) => ({ planned, implemented, pending }))(
      managers.find((r) => r.label === 'K Sreedhar') ?? /** @type {RollupRow} */ ({}),
    ),
  ...
  check(
    ...
    (({ implemented, pending }) => ({ implemented, pending }))(
      products.find((r) => r.label === 'Hei-VAP') ?? /** @type {RollupRow} */ ({}),
    ),
  ...
  check(
    ...
    (({ implemented, pending }) => ({ implemented, pending }))(
      campaigns.find((r) => r.label === 'Carousel demo') ?? /** @type {RollupRow} */ ({}),
    ),
  ...
  check(
    ...
    (({ planned, implemented, pending }) => ({ planned, implemented, pending }))(
      q1Principals.find((r) => r.label === 'Heidolph') ?? /** @type {RollupRow} */ ({}),
    ),
  ...
  check(
    ...
    (({ planned, implemented }) => ({ planned, implemented }))(
      q1Principals.find((r) => r.label === 'ThalesNano') ?? /** @type {RollupRow} */ ({}),
    ),
  ...
  const anonPosts = await db.query(`select count(*)::int as count from posts`)
  check('anon can read posts', (anonPosts.rows[0]?.count ?? 0) > 0, true)

  const anonRollup = await db.query(`select * from dashboard_rollup(2025)`)
  ...
  const adminUsersRls = await fresh.query(
  ...
  if (seedSql !== null) {
    ...
    const unassigned = await fresh.query(
    ...
    const colours = await fresh.query(
    ...
    // app — the same guarantee scripts/seed.js makes.
    ...
    const afterRerun = await fresh.query(
      `select
         (select count(*)::int from public.principals) as principals,
