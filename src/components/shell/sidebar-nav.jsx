...
/**
 * Sidebar navigation. The active item is marked by a 3px forest left rule
 * and forest text — not a filled background block, which would read as a
 * button.
 *
        ...
        return (
          <Link
            ...
            className={cn(
              ...
              active ? 'font-medium text-forest' : 'text-muted hover:bg-hover hover:text-ink',
            )}
          >
            {active && <span aria-hidden className="absolute inset-y-1 left-0 w-[3px] rounded-chip bg-forest" />}
            <item.icon
              aria-hidden
              strokeWidth={active ? 2 : 1.5}
              className={cn('size-4 shrink-0', active ? 'text-forest' : 'text-current')}
            />
            {item.label}
          </Link>
        )
