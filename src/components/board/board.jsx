 ...
 *
 * @param {{ posts: PostRow[], principals: PrincipalOption[] }} props
 */
export function Board({ posts, principals }) {
    ...
    <>
      ...
      {selected && (
        <PostClickTarget
          ...
          onSaved={(post) => setItems((current) => current.map((p) => (p.id === post.id ? post : p)))}
        />
      )}
    </>
  )
